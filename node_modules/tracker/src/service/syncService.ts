        ...
        for (const key of eventKeys) {
          try {
            ...
            await client.query(
              ...
              `,
              [event.campaign_id, event.subscriber_id, event.eventType, event.timeStamp]
            )
  ...
  async syncCounter(): Promise<void> {
    ...
    try {
      ...
      if (counterKeys.length === 0) {
        console.log('No counters to sync')
        return
      }
      ...
      for (const key of counterKeys) {
        ...
        const opens = openStr ? Number(openStr) : 0

        console.log(`Campaign ${campaignId}: ${opens} opens in cache`)
      }
